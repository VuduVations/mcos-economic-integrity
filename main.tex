...
\date{August 2026 \quad (preprint v0.2.4)}
...
spend bought attempts whose output was discarded --- and 228,889 of
228,890 discarded output tokens in the study were reasoning tokens, the
spend category no scoreboard sees. Rescue effectiveness was also non-stationary: a
...
65.1\% of all billed output tokens were reasoning tokens, and 228{,}889
of 228{,}890 discarded output tokens were reasoning tokens: the attempts
the pipeline threw away consisted almost entirely of thinking no one
ever saw.
...
companion paper's reported figures. And the delta between ledger and
...
twenty per-launch result files, and the compiled report are all
committed alongside it (Section~\ref{sec:artifacts}).
...
precise (we use the term descriptively, not as a formal equivalence
test: exact answer-key totals differ by cell and $n{=}10$ supports no
stronger claim than that neither measurement orders the cells). On capability, answer-key totals ran 468--474 of 480 checks
...
a scoreboard equivalent to the cells that discarded effectively none.}
\label{fig:econ}
\end{figure}
...
\textbf{(3) Nearly every discarded output token was a reasoning token.}
Summed over all cells and layers, discarded attempts billed 228{,}890
output tokens, of which 228{,}889 were reasoning tokens. A starved
attempt spends its entire completion budget thinking and emits nothing;
nearly all discarded output tokens were invisible reasoning tokens. Overall, 65.1\% of the study's billed
...
prevention and recovery as forecasting risk. What prevention
stabilizes is the \emph{attempt count at a configured budget}: one
attempt per job, with the starvation residual (measured tonight at 1
in 127) as the forecast error on that count. Realized dollars still
move with output length, cache participation, and provider list
prices --- prevention removes the attempt-count term from the bill's
variance, not the whole bill. A recovery-based configuration's cost
model additionally contains the rescue rate as a load-bearing
variable --- and that variable just
...
scoreboard through retries has an attempt count that is a function of
provider weather; one that meets it through headroom has an attempt
count that is a function of its own configuration. At month scale,
the first adds a random variable to the bill; the second leaves the
bill's variance where every pipeline already carries it --- in output
length and prices.
...
\$1.67 across the run --- the observed aggregate account-billed amount
under the controlled run conditions, attributable to nothing smaller
than the run. The \$0.62 residual between the bases is a measured
pricing residual, consistent with the provider's prompt-cache discount
structure; where provider pricing terms are independently known it may
be decomposed among realized cache discounts and other provider-side
pricing adjustments, which the per-attempt hit/miss splits support
...
Reasoning tokens were 65.1\% of this study's billed output and
228{,}889 of its 228{,}890 discarded output tokens. A cost dashboard that reports visible
...
per-repetition boards and agent modes, and the compiled report
(\texttt{econ\_report\_2026-08-10.json}) from which every number
...
The analyzer itself is a direct implementation of the definitions
given in this paper and is retained privately as part of the
measurement implementation (see the patent notice in the repository);
its SHA-256 digest is recorded in the repository's published manifest,
and every reported quantity is independently recomputable from the
published ledgers and result files using those definitions.
...
accounting, which we record in the spirit of the method). The frozen
artifact bundle whose SHA-256 digest is recorded in the related
provisional patent application is retained privately as evidence; the
public repository carries the data, the manifest, and this manuscript.
