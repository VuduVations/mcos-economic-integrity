...
\date{August 2026 \quad (preprint v0.1)}
...
armed. We define two metrics over the ledger: the \emph{Invisible Attempt
Factor} (\iaf{}), physical attempts per logical job, and the
\emph{Economic Contamination Index} (\eci{}), total spend over spend on
attempts whose output was actually used. All four cells scored 9--10/10
on a fixed 48-check benchmark; underneath, the attempt machinery differed
by up to a factor of $1.7$. Proactive budget headroom \emph{measures} as
prevention: \iaf{} $=1.000$ in both layers (127 agent jobs, 127
attempts, one empty reply). Retry-based recovery reaches the same
scoreboard with \iaf{} up to $1.70$ and \eci{} up to $1.96$ in the
...
(Section~\ref{sec:framework}): the Invisible Attempt Factor (\iaf{}),
physical attempts per logical job, computed per pipeline layer and never
pooled across layers; and the Economic Contamination Index (\eci{}),
total spend over spend on used attempts, where \emph{used} is defined
exactly (the final attempt of a logical job, when non-empty).
...
scored 9--10/10; the cells' \iaf{} and \eci{} separated cleanly, and in
the direction the companion paper's event counts predicted.
\item \textbf{Three economic findings}: prevention is measurable as
\iaf{} $= 1.000$ rather than inferable from absence of log lines
...
\textbf{Invisible Attempt Factor.} For a set of logical jobs $J$ with
physical attempts $A$,
\[
\iaf{} = \frac{|A|}{|J|}.
\]
...
scoreboard-equivalent: every cell graded 9--10 of 10 repetitions clean
on the 48-check benchmark (Table~\ref{tab:tiers}). The attempt
machinery underneath was not equivalent.
...
\begin{tabular}{lcccr}
\toprule
Cell & First-attempt clean & Recovered clean & Degraded & Mean agent \$/rep \\
\midrule
reactive & 0 & 9 & 1 & 0.064--0.067 \\
proactive & 9 & 0 & 1 & 0.065--0.068 \\
pure retry & 0 & 9 & 1 & 0.055--0.066 \\
bundle & 8 & 2 & 0 & 0.067--0.078 \\
\bottomrule
\end{tabular}
\caption{Per-repetition outcome tiers (ten-agent grading basis joined
to per-repetition ledger cost). Every cell's scoreboard is
9--10/10; only the bundle is 10/10 clean. Mean agent-layer cost per
repetition is nearly flat across cells and tiers at tonight's event
rates: the economic difference between the cells lives in
Table~\ref{tab:main}'s debate rows and in attempt-count variance, not
in the per-repetition agent bill.}
\label{tab:tiers}
\end{table}
...
the same scoreboard as the cells that discarded none.}
\label{fig:econ}
\end{figure}
...
119. Both cells graded 9/10. Nothing on the scoreboard, and nothing in
legacy returned-attempt metering, distinguishes them.
...
\textbf{Prevention measures as \iaf{} $=1.000$.} In the proactive cell
the companion paper's ``zero starvation events'' claim had rested on
the absence of a retry log line --- a line that \emph{cannot fire} in
that cell, because the retry is the thing ablated. Absence of evidence
from a disabled instrument is not evidence of absence. The ledger
replaces the inference with enumeration: 127 agent-layer jobs, 127
attempts, one empty reply (a zero-token transport error, a class no
completion budget can prevent); 119 debate-layer jobs, 119 attempts,
one starved. The claim survives direct measurement --- as 1-in-127,
not zero, which is how such claims should be stated.

\textbf{Recovery reaches the same scoreboard and pays for both
attempts.} The reactive cell graded 9/10, the same as proactive. To
...
retry --- was the only 10/10 cell, and its two first-attempt agent
starvations were both recovered by the retry at a discarded cost of
\$0.020. As in the companion study, the argument is not against retry;
...
retries, and outer re-asks --- to reach the same 9/10 scoreboard as
every other cell. Teams adding adversarial validation, self-critique,
...
\textbf{Report \iaf{} and \eci{} alongside accuracy.} Both metrics
...
A benchmark row of the form ``9/10 clean, agent \iaf{} 1.00, debate
\iaf{} 1.70, debate \eci{} 1.58'' says what ``9/10 clean'' hides:
...
only count: prevention measured as an \iaf{} of exactly one rather
than inferred from a silent log, a validation layer quietly running
at 1.7 times its diagrammed price,
...
in this paper is drawn. The instrumented client and the accompanying
